...
/*   Updated: 2026/07/06 00:32:20 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec2	calc_sphere_uv(t_sphere const *sphere, t_vec3 point)
{
	...
	phi = atan2f(vec3_dot(normal, sphere->onb.v), \
					vec3_dot(normal, sphere->onb.u));
	theta = asinf(fminf(1.0f, fmaxf(-1.0f, vec3_dot(normal, sphere->onb.w))));
	return ((t_vec2){\
