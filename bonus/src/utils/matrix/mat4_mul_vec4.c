...
t_vec4	mat4_mul_vec4(t_mat4 const *m, t_vec4 v)
{
	...
	result.x = vec4_dot(\
			vec4(m->m[0][0], m->m[0][1], m->m[0][2], m->m[0][3]), v);
	result.y = vec4_dot(\
			vec4(m->m[1][0], m->m[1][1], m->m[1][2], m->m[1][3]), v);
	result.z = vec4_dot(\
			vec4(m->m[2][0], m->m[2][1], m->m[2][2], m->m[2][3]), v);
	result.w = vec4_dot(\
			vec4(m->m[3][0], m->m[3][1], m->m[3][2], m->m[3][3]), v);
	return (result);
}
