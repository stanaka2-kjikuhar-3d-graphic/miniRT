...
bool	bvh_shading(t_ray const *ray, float dist)
{
	...
	if (isnan(t) || dist <= t)
		return (false);
	return (shadow_traverse_node(ray, dist, root));
}

/* any-hit: stops at the first blocker instead of keeping the nearest one */
static bool	shadow_traverse_node(\
	t_ray const *ray, float dist, t_bvh_node const *node)
{
	...
	{
		left = get_bvh_node(node->branch.left);
		t = calc_aabb_intersection(&(left->aabb), ray);
		if (t < dist && shadow_traverse_node(ray, dist, left))
			return (true);
		right = get_bvh_node(node->branch.right);
		t = calc_aabb_intersection(&(right->aabb), ray);
		return (t < dist && shadow_traverse_node(ray, dist, right));
	}
}
